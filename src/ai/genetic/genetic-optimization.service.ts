...
import {
  EngagementPredictor,
  ViralScore,
} from "../intelligence/engagement-predictor";
...
export class GeneticOptimizationService {
  ...
  async optimizeScript(
    ...
    try {
      ...
      const finalScoreResult = await this.predictor.predictViralScore(
        scriptText,
        topic,
      );
      ...
      this.logger.error(
        `❌ Genetic Optimization Failed for topic: "${topic}"`,
        error,
      );
      throw error;
    }
  ...
  async batchOptimize(
    ...
    this.logger.log(
      `✅ Batch optimization complete. Average score: ${avgScore.toFixed(1)}/100`,
    );
  ...
  async getOptimizationRecommendations(topic: string): Promise<{
    ...
    const estimatedDuration =
      recommendedGenerations * recommendedPopulationSize * 5000; // ~5s per evaluation

    // Estimate API calls (script generation + scoring)
    const estimatedCost =
      recommendedGenerations * recommendedPopulationSize * 2; // 2 calls per individual
