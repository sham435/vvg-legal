...
export class VyroService {
   ...
   */
  async generateImage(prompt: string, style: string = 'realistic', aspectRatio: string = '1:1', seed?: string): Promise<any> {
    ...
    try {
      const response = await axios.post('https://api.vyro.ai/v2/image/generations', form, { headers });
   ...
   */
  async generateTextToVideo(prompt: string, style: string = 'kling-1.0-pro'): Promise<any> {
    const form = new FormData();
    form.append('prompt', prompt);
    form.append('style', style);
    ...
    const headers = { ...form.getHeaders(), ...this.getAuthHeader() };
    try {
      const response = await axios.post('https://api.vyro.ai/v2/video/text-to-video', form, { headers });
      return response.data;
    } catch (error) {
   ...
   */
  async generateImageToVideo(prompt: string, style: string = 'kling-1.0-pro', imagePath: string): Promise<any> {
    const form = new FormData();
    form.append('prompt', prompt);
    form.append('style', style);
