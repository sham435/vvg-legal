...
export class AiService {
   ...
   */
  async generateScript(topic: string, description?: string): Promise<VideoScript> {
    if (!this.openai) {
...
Keep it under 200 characters.`;
   ...
   */
  async optimizeTitle(originalTitle: string, platform: 'youtube' | 'tiktok' | 'instagram'): Promise<string> {
    if (!this.openai) {
